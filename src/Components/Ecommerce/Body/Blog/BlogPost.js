...
import { FaShareAlt, FaArrowLeft, FaCalendarAlt, FaUser, FaCheck, FaHeart, FaRegHeart, FaEye } from 'react-icons/fa';
...
const ProductEmbed = ({ id, options }) => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try { const data = await blogServices.getProductById(id); setProduct(data); } 
            catch (error) { } finally { setLoading(false); }
        };
        if (id) fetchProduct();
    }, [id]);
    if (loading) return <div className="blog-product-loader">Carregando oferta...</div>;
    if (!product) return null;
    const price = product.value?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    const image = product.mediaUrls?.[0] || "https://placehold.co/150";
    const showImg = options.img === 'true'; const showName = options.name === 'true'; const showPrice = options.price === 'true'; const showBtn = options.btn === 'true';
    return (
        <a href={`/product/${id}`} target="_blank" rel="noopener noreferrer" className="product-dynamic-card">
            {showImg && <div className="pdc-image"><img src={image} alt={product.name} /></div>}
...
const BlogPost = () => {
    ...
    useEffect(() => {
        if(post) {
            ...
            if (post.likesCountOverride !== undefined) setLikeCount(post.likesCountOverride);
            else if (post.likes) setLikeCount(post.likes.length);

            if (post.viewsCountOverride !== undefined) setViewCount(post.viewsCountOverride);
    ...
    useEffect(() => {
        if(post && id && !hasRegisteredView.current) {
            ...
            const alreadyViewedById = userId && post.views?.some(v => v.clientId === userId);
    ...
    const renderPostContent = (content) => { 
        ...
        const regex = /\[\[PRODUCT:id=(\d+)\|img=(true|false)\|name=(true|false)\|price=(true|false)\|btn=(true|false)\]\]/g;
        ...
        while (i < parts.length) {
            ...
            if (textSegment) {
                ...
                elements.push(<div key={`txt-${i}`} dangerouslySetInnerHTML={{ __html: formattedText }} className="post-text-block"/>);
            }
            if (i + 5 < parts.length && /^\d+$/.test(parts[i+1])) {
                const pId = parts[i+1];
                const opts = { img: parts[i+2], name: parts[i+3], price: parts[i+4], btn: parts[i+5] };
                elements.push(<div key={`prod-${i}`} className="blog-embedded-wrapper"><ProductEmbed id={pId} options={opts} /></div>);
                i += 6; 
            } else { i++; }
        }
        return elements;
    ...
    if (error || !post) return <div className="blog-error-container"><h2>Post não encontrado</h2><Link to="/blog" className="back-btn">Voltar</Link></div>;
    const dateFormatted = new Date(post.createdAt || post.dateCreated).toLocaleDateString('pt-BR', { day: 'numeric', month: 'long', year: 'numeric' });
        ...
        <div className="blog-post-page">
            <header className="post-detail-hero">
                ...
                <div className="post-hero-content">
                    <div className="post-category-badge">{post.category?.categoryName || 'Artigo'}</div>
                    <h1 className="post-detail-title">{post.title}</h1>
                    <div className="post-detail-meta">
                        <span><FaUser /> Gemas Brilhantes</span>
                        <span><FaCalendarAlt /> {dateFormatted}</span>
                        <span><FaEye /> {viewCount} visualizações</span>
                        <span style={{color: isLiked ? '#ff4757' : 'inherit', cursor: 'pointer'}} onClick={handleLike}>
                             {isLiked ? <FaHeart /> : <FaRegHeart />} {likeCount} curtidas
                        </span>
                    </div>
                </div>
            </header>
            ...
            <main className="post-detail-container">
                <div className="post-detail-actions">
                    <Link to="/blog" className="back-link"><FaArrowLeft /> Voltar</Link>
                    <div className="right-actions" style={{display:'flex', gap:'10px'}}>
                         <button className={`share-btn ${isLiked ? 'liked-active' : ''}`} onClick={handleLike} style={{background: isLiked ? '#ffeaa7' : '#F3F4F6', color: isLiked ? '#d63031' : '#122C4F'}}>
                             {isLiked ? <FaHeart /> : <FaRegHeart />} {likeCount}
                         </button>
                        <button className={`share-btn ${linkCopied ? 'copied' : ''}`} onClick={handleShare}>
                            {linkCopied ? <><FaCheck /> Link Copiado</> : <><FaShareAlt /> Compartilhar</>}
                        </button>
                    </div>
                </div>
                ...
                {post.redirectLink && (
                    <div className="post-detail-cta">
                        <a href={post.redirectLink} target="_blank" rel="noopener noreferrer" className="cta-button">
                            Saiba Mais
                        </a>
                    </div>
                )}
