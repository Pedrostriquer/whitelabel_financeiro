...
import blogAddService from "../../../../dbServices/blogAddService";
...
const BlogPostCard = ({ post, openAuthModal }) => {
  ...
  useEffect(() => {
    ...
    const alreadyViewedById =
      currentUserId && post.views?.some((v) => v.clientId === currentUserId);
  ...
  const date = new Date(post.createdAt || post.dateCreated).toLocaleDateString(
    "pt-BR",
    {
      day: "numeric",
      month: "long",
      ...
      <div className="horizontal-card-content">
        <div className="card-header">
          ...
          <div className="card-actions-top">
            <button
              className={`card-share-btn ${isCopied ? "copied" : ""}`}
              onClick={handleShare}
            >
        ...
        <p className="card-excerpt">{getExcerpt(post.text)}</p>
...
const Blog = () => {
  ...
  const [blogAdd, setBlogAdd] = useState(null);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setCategories(cats || []);
        const adds = await blogAddService.getBlogAdds();
  ...
  return (
    <div className="blog-page-wrapper">
        ...
        <div className="tag-filters-container">
          ...
          {categories.map((cat) => (
            <button
              key={cat.id}
              className={`tag-filter-btn ${
                selectedTag === cat.categoryName ? "active" : ""
              }`}
              onClick={() => setSelectedTag(cat.categoryName)}
            >
              {cat.categoryName}
            </button>
          ))}
      ...
      <main className="blog-layout-container">
        {blogAdd && (
          <aside className="blog-sidebar side-left">
            <div className="sticky-add">
              <img
                src={blogAdd.imageUrl}
                alt={blogAdd.name}
                style={{ width: blogAdd.width, height: blogAdd.height }}
              />
            </div>
          </aside>
        )}
        ...
        {blogAdd && (
          <aside className="blog-sidebar side-right">
            <div className="sticky-add">
              <img
                src={blogAdd.imageUrl}
                alt={blogAdd.name}
