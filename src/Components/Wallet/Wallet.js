...
const AnimatedInfoItem = ({ label, brlValue, usdValue, isLoading }) => {
  ...
  const { currentValue: currentBrlValue, animatedColor } = useCountUpAnimation(
    brlValue,
    3000,
    isLoading,
    endColorRgb
  );
  const { currentValue: currentUsdValue } = useCountUpAnimation(
    usdValue,
    3000,
    isLoading,
    [108, 117, 125]
    ...
    return (
      <div style={style.infoPanelContainer}>
        <div style={style.infoPanelBody}>
          <p>Carregando regras de saque...</p>
        </div>
      </div>
    );
  }
    ...
    <div style={style.infoPanelContainer}>
      ...
      <div style={style.infoPanelBody}>
        ...
        <div style={style.infoPanelRow}>
          ...
          <span style={style.infoPanelValue}>
            Das {rules.startHour} às {rules.stopHour}
          </span>
        </div>
        <div style={style.infoPanelRow}>
...
export default function Wallet() {
  ...
  const checkWithdrawalWindow = (rules) => {
    ...
    return (
      currentTime >= startTimeInMinutes && currentTime <= stopTimeInMinutes
    );
  };
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        const [withdrawalsData, walletData, rulesData, rateData] =
          await Promise.all([
  ...
  const handleRequestCode = async (e) => {
    ...
    if (amount < withdrawRules.minimumToWithdraw) {
      alert(
        `O valor mínimo para saque é de ${formatServices.formatCurrencyBR(
          withdrawRules.minimumToWithdraw
        )}.`
      );
      return;
    }
  ...
  const saqueBtnStyle = {
    ...
    ...(buttonDisabled
      ? { backgroundColor: "#a5d6a7", cursor: "not-allowed" }
      : {}),
  };
          ...
          <div style={style.walletMainActions}>
            ...
            <div style={style.actionButtons}>
              <button
                ...
                title={
                  !isWithdrawalOpen
                    ? "Saques estão disponíveis apenas no dia e horário estipulados"
                    : ""
                }
              >
                Realizar Saque
            ...
            <div style={style.infoBlock}>
              ...
              <div style={style.infoRow}>
                <AnimatedInfoItem
                  ...
                  usdValue={walletInfo.totalBalance / dolarRate}
                  isLoading={isWalletLoading}
                />
                <AnimatedInfoItem
                  label="VALOR DISPONÍVEL"
                  brlValue={walletInfo.totalAvaliableBalance}
                  usdValue={walletInfo.totalAvaliableBalance / dolarRate}
                  isLoading={isWalletLoading}
                />
              </div>
            </div>
            <div style={{ ...style.infoBlock, marginBottom: 0 }}>
              ...
              <div style={style.infoRow}>
                <AnimatedInfoItem
                  ...
                  usdValue={walletInfo.totalIncome / dolarRate}
                  isLoading={isWalletLoading}
                />
                <AnimatedInfoItem
                  label="VALOR SACADO"
                  brlValue={walletInfo.totalWithdraw}
                  usdValue={walletInfo.totalWithdraw / dolarRate}
                  isLoading={isWalletLoading}
                />
              </div>
            </div>
          ...
          <form onSubmit={handleRequestCode}>
            ...
            <div style={style.modalBody}>
              ...
              <div style={style.inputGroup}>
                ...
                <button
                  type="button"
                  style={style.btnTotal}
                  onClick={() =>
                    setSaqueAmount(walletInfo.totalAvaliableBalance.toFixed(2))
                  }
                >
                  TOTAL
                </button>
              ...
              <p style={style.modalInfoText}>
                Será cobrada uma taxa de {withdrawRules?.fee || 0}% em cima do
                saque
              </p>
              <p style={style.modalInfoText}>
                Valor mínimo para saque:{" "}
                {formatServices.formatCurrencyBR(
                  withdrawRules?.minimumToWithdraw || 0
                )}
              </p>
            </div>
